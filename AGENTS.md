...
本仓库维护一套面向 Coding Agent 的中文研发流程与可组合工程 Skill。根目录 `README.md` 面向使用者；本文件约束维护本仓库的 Agent。

## 目录结构

- 所有可安装 Skill 必须平铺在 `engineering-skills/<skill-name>/`，不要重新引入 `skills/`、`engineering/`、`productivity/`、`flows/` 等中间层。
- `engineering-skills/` 的每个一级子目录都必须是一个完整 Skill，并直接包含 `SKILL.md`。
...
- 封装完整研发链路的 Skill 使用 `flow-*` 前缀；可自由组合的阶段能力保持独立，不增加分类目录。
- 流程 Skill 只负责阶段编排、门禁和上下文交接。具体阶段的详细规则留在对应模块 Skill 中，避免复制后产生两套规范。
- 除根 `README.md` 的来源说明和 `LICENSE` 中的版权声明外，不重新引入 Matt Pocock 个人品牌。

## 修改时保持同步

- 新增、重命名或删除 Skill 时，同步更新根 `README.md`、`engineering-skills/README.md` 以及相关 Skill 间的引用。
- 同步检查 `engineering-skills/install-skills/SKILL.md` 中的扫描规则、预期数量和安装说明。
...
- `engineering-skills/` 的每个一级子目录都直接包含 `SKILL.md`，且不存在更深层的 Skill。
